...
d = {'porosity': por, 'dP': dP, 'k': k, 'Re': Re, 'eps_s': eps_s, 'ssa_bulk': ssa_bulk, 'ssa_solid': ssa_solid}
...
out = '{},'.format(case) + '{:.0f},'.format(ppi) + '{:.2f},'.format(por) + '{:.5e},'.format(dP)+ '{:.5e},'.format(k) + '{:.5e},'.format(Re) + '{:.2f},'.format(
...
if out_f in os.listdir(cwd.parent):
    ...
    with open(resultsPath, 'w+') as res_f:
        res_f.write('case,ppi,por,dP,k,Re,eps_s,ssa_bulk,ssa_solid')
        res_f.write('\n')
        res_f.write(out)
